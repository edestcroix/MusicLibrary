...
class ProgressMonitor:
    ...
    def _run(self):
        while not self._stop_event.is_set():
            ...
            self._update_scale()
        self._update_widgets(0, '0:00', '0:00')
    ...
    def _update_scale(self):
        if self._player.state == 'playing':
            ...
            self._update_widgets(progress, time_str, duration_str)

    def _update_widgets(self, value, progress, duration):
        GLib.idle_add(self._scale.set_value, value)
        GLib.idle_add(self._start_label.set_text, progress)
        GLib.idle_add(self._end_label.set_text, duration)
